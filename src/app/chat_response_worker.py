            ...
            messages.append(
                {
                    ...
                    "content": (
                        f"[현재 시각: {now_str}]\n\n"
                        f"사용자 질문: {last_user_message}\n\n"
                        "아래는 최신 웹 검색 문맥입니다. 이를 바탕으로 정보의 신뢰성을 높이기 위해 답변의 마지막에 반드시 마크다운 링크 형식('[웹사이트명](출처URL)')으로 출처를 명시하여 클릭할 수 있게 만드세요.\n"
                        "또한 직역투나 중복 표현을 피해 자연스럽고 매끄러운 한국어로 답변해 주세요.\n"
                        "화씨(°F) 같은 미국식 단위가 문맥에 포함되어 있다면 반드시 계산을 통해 한국 기준(°C, km 등)으로 변환하여 문장에 자연스럽게 녹여내세요.\n\n"
                        f"[검색 결과]\n{search_result}"
                    ),
                }
            )
