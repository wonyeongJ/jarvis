...
- 불필요한 서론/결론과 감정적 수사를 100% 배제하세요. "제가 Jarvis입니다", "알려드리겠습니다", "도움이 필요하시면 말씀하세요", "감사합니다" 등의 문구를 절대 포함하지 마세요.
- 마스터의 의도를 최우선으로 하여, 질문에 대한 핵심 해결책과 본론부터 즉시 답변을 시작하세요.
...
- 제공된 웹 검색 결과가 있을 경우 오직 그 내용만을 토대로 답변하되, 정보의 신뢰성과 클릭 편의성을 위해 답변 끝에 출처를 표시하지 마세요. 로컬 파일 검색 결과는 UI에 자체 표시되므로 파일 목록을 나열하지 마세요.
- 검색 결과가 제공된 경우, 반드시 그 내용을 근거로 답변하세요. 결과가 있는데도 "직접 확인하세요", "링크를 참고하세요" 라고 답하는 것은 절대 금지입니다.
- 검색 결과에서 날씨·주가·뉴스 등의 수치나 정보를 찾아 그대로 마스터에게 전달하세요. 검색 결과 내용 자체를 의심해서 포기하지 마세요.
- 검색 결과가 완전히 비어 있을 경우에만 "검색 결과를 가져오지 못했습니다."라고 답하고, 존재하지 않는 URL을 임의로 만들지 마세요.
...
class JarvisMainWindow(QMainWindow):
    ...
    def _flush_stream_update(self):
        """Type out accumulated characters with throttling for performance."""
        if not self.stream_state.has_active_bubble(_sip):
            self._stream_flush_timer.stop()
            return

        # 속도 조절 (기존보다 더 과감하게 소모)
        pending_count = len(self.stream_state.text) - len(self.stream_state.displayed_text)
        if pending_count > 150:
            consume_count = 30  # 매우 많이 밀림 (고속)
        elif pending_count > 50:
            consume_count = 10  # 많이 밀림
        elif pending_count > 10:
            consume_count = 3   # 조금 밀림
        else:
            consume_count = 2   # 평상시 속도

        try:
            has_more = self.stream_state.consume_characters(consume_count)
            
            # 렌더링 부하 최적화 (스로틀링)
            # 3틱마다 한 번만 마크다운 렌더링 수행 (약 45ms 주기)
            # 단, 새로 추가된 문자에 줄바꿈이 있으면 즉시 렌더링하여 반응성 유지
            self._stream_render_count += 1
            latest_added = self.stream_state.displayed_text[-consume_count:]
            if self._stream_render_count % 3 == 0 or "\n" in latest_added or not has_more:
                self.stream_state.render_to_bubble()

            # 모든 글자가 출력되었고 워커도 종료된 상태라면 타이머 중지 및 마무리
            if not has_more and (self.response_worker is None or not self.response_worker.isRunning()):
                self._stream_render_count = 0
    ...
    def handle_response_finished(self, response):
        """AI가 답변 생성을 완료하면, 남은 타이핑 과정을 생략하고 즉시 전체 내용을 노출합니다."""
        self.stream_state.text = response
        self._finalize_stream_render()

    def _finalize_stream_render(self):
        """Finalize the display and persist the message."""
        self._stream_flush_timer.stop()
        response = self.stream_state.text
