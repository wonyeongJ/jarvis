...
### Q. 최근 유행하는 LangChain 같은 프레임워크를 왜 사용하지 않았나요?
- **응답 대기 시간 최소화**: LLM이 질문의 의도를 분석하기 위해 '생각'하는 단계를 최대한 생략하고 즉각 답변하기 위해, 직접 구현한 정규식 기반 라이트웨이트 라우팅을 채택했습니다.
- **빌드 및 배포 효율화**: 특정 기능에 최적화된 최소한의 라이브러리만 사용하여 PyInstaller 빌드 시간을 단축하고, 실행 파일(`.exe`) 최종 용량을 가볍게 유지하는 데 중점을 두었습니다.
- **유지보수와 제어권**: 프레임워크의 추상화 계층 없이 코드와 프롬프트 흐름을 직접 제어하므로, 비서 페르소나와 답변 일관성을 더 세밀하게 관리할 수 있습니다.
